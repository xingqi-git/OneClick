...
### 6. 服务器检查
- 多服务器健康状态批量检查
- 支持连通性检测（ping）、SSH 登录验证、Root SSH 权限检查
- 系统时间同步校验（可配置误差容忍范围）、防火墙状态检测
- 自定义命令回显检查（支持"包含" / "不包含"两种匹配模式）
- 检查结果实时更新，绿色 ✔️ 通过 / 红色 ❌ 不通过 / ⏸️ 跳过
- 支持单服务器串行检查（前面检查失败，后面自动跳过）
- 运行过程可随时停止，关闭窗口时有确认提示
...
- `停止监控`：终止监控进程
- `查看数据`：打开图表窗口查看历史监控数据
- `下载数据`：从远程服务器下载最新监控数据到本地（仅远程模式）
- `清除数据`：可选择清除本地、服务器或全部监控数据

### 5. 弱网模拟

选择 `快捷按钮类型选择 `弱网`，配置服务器信息（包括文件暂存路径）后生成快捷按钮。点击按钮进入弱网控制面板：

1. 选择目标网卡（自动获取服务器网卡列表，支持手动刷新）
2. 编辑规则：设置延迟、丢包率、带宽限制等参数，以及持续时长和间隔时长
3. 将规则添加到队列，可调整顺序或删除
4. 设置循环次数（0 表示无限循环）
5. 点击 `开始弱网` 执行，脚本将在服务器后台运行
6. 点击 `查看运行日志` 可查看实时进度和历史运行记录（日志会自动下载到本地）
...
选择 `快捷按钮类型选择 -> 服务器检查`，配置要检查的服务器及检查项：

1. 点击 `添加服务器列` 选择要检查的服务器（可多选）
2. 在表格中为每台服务器配置检查项：
   - **连通**：检测网络是否可达（期望"是"或"否"）
   - **SSH登录**：验证 SSH 连接是否正常
   - **Root SSH权限**：检查 PermitRootLogin 配置
   - **系统时间**：服务器时间与本地时间的误差容忍（分钟）
   - **防火墙**：检测防火墙是否启用
   - **命令回显N**：自定义命令及期望的输出内容（包含/不包含）
3. 填写按钮名称后点击 `生成快捷按钮`
4. 点击生成的快捷按钮进入检查面板，将自动开始执行所有检查
5. 检查结果会逐项实时更新（绿色 ✔️ 表示通过，红色 ❌ 表示不通过）

提示：如某一项检查失败（如网络不可达），后续检查项会自动标记为"网络不可达"或"SSH不可用"，避免重复检测。正在检查时可随时点击 `停止检查` 中止，关闭窗口时会有确认提示。
