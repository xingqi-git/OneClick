...
def build_remote_windows_clean_cmd(remote_dir, start_str, end_str):
    """
    构造 Windows 远程清理的 PowerShell 命令
    按时间字符串比较，与本地逻辑类似
    """
    # 因为远程 Windows 通过 SSH 执行 powershell 较复杂，这里提供一个 PowerShell 脚本块
    # 实际使用时用 powershell -Command 执行
    ps_script = f'''
$dir = "{remote_dir}"
$start = "{start_str}"
$end = "{end_str}"
...
if (-not (Test-Path $dir)) {{
    Write-Output "DIR_NOT_FOUND"
    return
}}

$logFiles = Get-ChildItem -Path $dir -Filter "*.log" | Where-Object {{ $_.Name -ne "OneClickMonitor.log" }}
...
foreach ($f in $logFiles) {{
    ...
    $firstCol = ($lines[1] -split ',')[0].Trim()
    $lastCol = ($lines[-1] -split ',')[0].Trim()

    if ([string]::IsNullOrWhiteSpace($firstCol) -or [string]::IsNullOrWhiteSpace($lastCol)) {{
        continue
    }}

    # 完全在区间之前或之后
    if ($lastCol -lt $start -or $firstCol -gt $end) {{
        continue
    }}

    # 完全在区间内，删除
    if ($firstCol -ge $start -and $lastCol -le $end) {{
    ...
    for ($i = 1; $i -lt $lines.Count; $i++) {{
        $t = ($lines[$i] -split ',')[0].Trim()
        if ($t -lt $start -or $t -gt $end) {{
            $outLines += $lines[$i]
        }}
    }}

    if ($outLines.Count -le 1) {{
        Remove-Item -Path $path -Force
        $deleted++
    }} else {{
        $outLines | Out-File -FilePath $tmp -Encoding utf8
        Move-Item -Path $tmp -Destination $path -Force
        $trimmed++
    }}
}}

Write-Output "RESULT:deleted=$deleted:trimmed=$trimmed"
'''
    return ps_script
